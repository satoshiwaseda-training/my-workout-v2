...
# --- 5. メニュー生成 (生成した瞬間にSession Stateにロック) ---
st.markdown("---")
...
if st.button("🚀 最新エビデンスに基づきメニューを生成"):
    with st.spinner("サトシさんに最適な強度を算出中..."):
        api_key = st.secrets["GOOGLE_API_KEY"].strip()
        url = f"https://generativelanguage.googleapis.com/v1/models/gemini-2.0-flash:generateContent?key={api_key}"
        
        system = (
            f"あなたはMuscle Mate。BP:{rpm_bp}kg基準。時間{t_limit}分。休憩(180秒/90秒)を算入。"
            f"重要：重量は一般的で安全なRPE8を基準とし、1RMの60-75%程度で算出。"
            f"種目数を3種目に厳選せよ。形式：'種目名:重量kgx回数xセット数[休憩:秒]'"
        )
        ...
        if res.status_code == 200:
            ...
            st.session_state['active_tasks'] = parsed # ここで固定
            st.rerun() # 確実に描画を更新
    ...
    with st.form("stable_workout_record_form"):
        ...
        if st.form_submit_button("🔥 実績をGoogle Driveへ保存"):
            if sheet and all_logs:
                ...
                st.balloons()
                st.success(f"総負荷 {total_vol}kg を保存しました！")
