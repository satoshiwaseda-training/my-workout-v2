...
st.markdown("""
    ...
    .set-row { background-color: rgba(255, 255, 255, 0.4); padding: 10px; border-radius: 10px; margin-bottom: 5px; }
    </style>
    """, unsafe_allow_html=True)

st.title("💪 Muscle Mate: Precision Analyst")
...
if 'ai_resp' in st.session_state:
    st.info(f"📋 {t_limit}分・最適化メニュー:\n{st.session_state['ai_resp']}")

# --- 5. 【完全復活】セット数に応じた個別入力欄 ---
if 'active_tasks' in st.session_state and st.session_state['active_tasks']:
    st.markdown("---")
    st.subheader(f"📝 実績記録 (セット別入力)")
    
    with st.form("workout_detailed_form"):
        all_logs = []
        total_vol = 0
        for i, task in enumerate(st.session_state['active_tasks']):
            ...
            for s_num in range(1, task['s'] + 1):
                ...
                with c_w: 
                    # 各セットごとに微調整可能
                    w = st.number_input(f"重量 (kg)", value=task['w'], key=f"w_{i}_{s_num}", step=2.5)
                with c_r: 
        ...
        if st.form_submit_button("🔥 全セット完了！Driveに同期"):
            if sheet and all_logs:
                now = datetime.now().strftime("%Y-%m-%d %H:%M")
                sheet.append_row([now, prog, ", ".join(targets), ", ".join(all_logs), f"Total:{total_vol}kg"])
                st.balloons()
                st.success(f"保存完了！総負荷 {total_vol}kg (軽自動車 {total_vol/1000:.2f}台分) を刻みました！")
