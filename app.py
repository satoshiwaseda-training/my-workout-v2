...
if st.button("AIメニュー生成 (EXECUTE WITH KNOWLEDGE)", type="primary"):
    ...
    prompt = f"ナレッジベース:{st.session_state.knowledge_base}\nユーザー制約:{st.session_state.custom_constraints}\nメイン:『{mode}』{target_w}kg,{r_info['sets']}set,{r_info['reps']}rep。部位:{parts} 形式：『種目名』 【重量kg】 (セット数) 回数 [休憩]"
    try:
...
with st.expander("🧪 知識ベース（AIが参照します）"):
