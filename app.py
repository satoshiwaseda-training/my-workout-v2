...
st.markdown("""
    ...
    .workout-card { background: rgba(255, 255, 255, 0.85); padding: 20px; border-radius: 15px; border-left: 10px solid #FF4B2B; margin-bottom: 20px; }
    </style>
    """, unsafe_allow_html=True)

st.title("💪 Muscle Mate: Real-time Dynamic Analyst")
...
if st.button("🚀 最新エビデンスに基づきメニューを生成"):
    with st.spinner("AIが現実的な強度を算出中..."):
        ...
        system = (
            f"あなたはMuscle Mate。BP:{rpm_bp}kg基準。時間{t_limit}分。休憩(180秒/90秒)を計算に含め、種目数を3種目に厳選せよ。"
            f"重要：重量は一般的で安全なRPE8を基準とし、1RMの60-75%程度で算出。"
        ...
        if res.status_code == 200:
            ...
            # セッションを確実に上書き更新
            st.session_state['active_tasks'] = [] # 一度クリア
            parsed = []
...
if 'active_tasks' in st.session_state and st.session_state['active_tasks']:
    st.info(f"📋 推奨プラン:\n{st.session_state.get('ai_resp_display', '')}")
