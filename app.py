...
if st.button("🚀 最新エビデンスに基づきメニュー生成"):
    with st.spinner("世界中の論文データを解析し、最適な強度を算出中..."):
        ...
        system = (
            ...
            f"解説禁止。'種目名:重量kgx回数xセット数'の形式のみ。重量は1RMの60-85%で算出せよ。"
        )
...
if 'active_tasks' in st.session_state and st.session_state['active_tasks']:
    ...
    with st.form("workout_detailed_form"):
        ...
        for i, task in enumerate(st.session_state['active_tasks']):
            st.markdown(f"#### 【{task['name']}】 (目標: {task['w']}kg)")
