...
with st.sidebar:
    ...
    st.markdown(f"""
        <p>SQ: <span style='color:#00E5FF;'>{st.session_state.history_log.get('スクワット', 0)}kg</span></p>
        <p>BP: <span style='color:#00E5FF;'>{st.session_state.history_log.get('ベンチプレス', 0)}kg</span></p>
        <p>DL: <span style='color:#00E5FF;'>{st.session_state.history_log.get('デッドリフト', 0)}kg</span></p>
    """, unsafe_allow_html=True)
...
# 1RM設定
with st.expander("👤 自己ベスト(1RM)の初期設定"):
...
# カレンダー表示
with st.expander("📅 LOG HISTORY (カレンダー履歴)"):
